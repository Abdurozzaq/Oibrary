<template>
  <div class="ma-5">
    ...
    <v-card>
      ...
      Daftar Pustakawan
        ...
        <v-text-field
          ...
          @click:append-outer="searchPustakawan"
          class="mb-3"
        ></v-text-field>
      </v-card-title>

      <v-data-table
        v-if="daftarPustakawan && daftarPustakawanFiltered == null"
        :headers="headers"
        :items="daftarPustakawan"
        :items-per-page="5"
        class="elevation-1"
      >
        ...
        <template v-slot:[`item.action`]="props">
          <v-tooltip bottom>
            ...
            <span>Edit Pustakawan</span>
          </v-tooltip>

          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-btn v-bind="attrs" v-on="on" class="ma-2" fab dark small color="red" @click.prevent="deletePustakawan(props.item)">
                <v-icon dark>mdi-trash-can-outline</v-icon>
              </v-btn>
            </template>
            <span>Delete Pustakawan</span>
          </v-tooltip>
      ...
      <v-data-table
        v-if="daftarPustakawan && daftarPustakawanFiltered != null"
        :headers="headers"
        :items="daftarPustakawanFiltered"
        :items-per-page="5"
        class="elevation-1"
      >
        ...
        <template v-slot:[`item.action`]="props">
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-btn v-bind="attrs" v-on="on" class="ma-2" fab dark small color="red" @click.prevent="deletePustakwan(props.item)">
                <v-icon dark>mdi-trash-can-outline</v-icon>
              </v-btn>
            </template>
            <span>Delete Pustakwan</span>
          </v-tooltip>
    ...
    <v-dialog v-model="editPustakwanDialog" fullscreen hide-overlay transition="dialog-bottom-transition">
      <v-card>
        <v-toolbar dark color="primary">
          <v-btn icon dark @click="editPustakwanDialog = false">
            <v-icon>mdi-close</v-icon>
          </v-btn>
          <v-toolbar-title>Edit Pustakwan</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn dark text @click="editPustakwan()">Save</v-btn>
          </v-toolbar-items>
        </v-toolbar>
        <v-card-text>
          <v-row>
  ...
  export default {
    ...
    methods: {
      getData: function() {
        ...
        axios.get('api/siAdmino/users/pustakawan/list')
      ...
      searchPustakawan: function() {
        let currentObj = this
        
        if (currentObj.search != null && currentObj.search.length != 0) {

          axios.post('api/siAdmino/users/pustakawan/search', {
      ...
      editPustakwan: function () {
        ...
        if (currentObj.$v.$invalid) {
          ...
          axios.post('api/siAdmino/users/pustakawan/edit/' + currentObj.edit_id, formData)
            .then(function (response) {
      ...
      deletePustakwan: function (item) {
        let currentObj = this
        currentObj.overlay = true

        axios.post('api/siAdmino/users/pustakawan/delete/' + item.id)
          .then(function (response) {
            currentObj.overlay = false
            currentObj.snack = true
            currentObj.snackColor = 'success'
